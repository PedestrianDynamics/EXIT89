...
10	ir = ir + 1
	...
	else if (its_arcs.eq..true. .or. its_nodes.eq..true.) then
		if (columns_found) then
			if (carray(ir,2).eq.needed_tag.and.tag_found.eq..false.) then
				ir_first_tag = ir
				tag_found = .true.
			else if (carray(ir,2).eq.'Endset') then
				if (tag_found.eq..false.) stop 'No beginning arc or node before Endset'
				ir_last_tag = ir - 1

	! we found everything we need ... set floors, columns, first row and last row in set, let's write stuff
